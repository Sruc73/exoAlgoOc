...
def majoritary_mentions_hash(candidates_results):
  r = {}
  for candidate, candidates_result in candidates_results.items():
    cumulated_votes = 0
    for mention, vote_count in enumerate(candidates_result):
...
def sort_candidates_by(mentions):
   ...
   while swapped:
     ...
     for j in range(0, len(unsorted) - 1):
       ...
       # (note that here we compare tuples, wich is pretty neat)
